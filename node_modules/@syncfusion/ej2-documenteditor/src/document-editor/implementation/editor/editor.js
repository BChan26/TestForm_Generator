...
var Editor = /** @class */ (function () {
     ...
     * @returns {BodyWidget}
     */
    Editor.prototype.insertSection = function (selection, selectFirstBlock, isUndoing, sectionBreakContinuous) {
        ...
        //Split body widget
        firstBlock = this.splitBodyWidget(lastBlock.bodyWidget, newSectionFormat, lastBlock, sectionBreakContinuous).firstChild;
        if (isUndoing) {
            this.layoutWholeDocument(true);
        }
     ...
     * @private
     */
    Editor.prototype.splitBodyWidget = function (bodyWidget, sectionFormat, startBlock, sectionBreakContinuous) {
        ...
        if (newBodyWidget.page === undefined) {
            ...
            bodyWidget.page.bodyWidgets.splice(bodyWidget.index + 1, 0, newBodyWidget);
        }
    ...
    Editor.prototype.insertImageInternal = function (imageString, isUiInteracted, width, height) {
        ...
        if (isNullOrUndefined(width) && isNullOrUndefined(height)) {
            ...
            var editor_1 = this;
            image.addEventListener('load', function () {
                editor_1.insertPicture(imageString, this.width, this.height, true);
            });
     ...
     *
     * @returns {void} Specify the style name to be created.
     */
    ...
    Editor.prototype.updateNextBlocksIndex = function (block, increaseIndex) {
        ...
        if (block.containerWidget instanceof BodyWidget) {
            ...
            if (block.containerWidget.page.bodyWidgets.length > 1) {
                ...
                for (var i = this.documentHelper.pages.indexOf(block.containerWidget.page); i < this.documentHelper.pages.length; i++) {
                    ...
                    for (var j = 0; j < page.bodyWidgets.length; j++) {
                        if (page.bodyWidgets[j].indexInOwner === fromSectionIndex) {
                            ...
                            nextIndex = 0;
                            if (!isNullOrUndefined(page.bodyWidgets[j].nextRenderedWidget) && page.bodyWidgets[j].nextRenderedWidget.sectionFormat.breakCode !== 'NoBreak') {
                                fromSectionIndex++;
                            }
                        }
                    }
    ...
    Editor.prototype.singleBackspace = function (selection, isRedoing) {
        ...
        if (offset === selection.getStartOffset(paragraph) && selection.start.currentWidget.isFirstLine()) {
            ...
            var paragraphInfo = this.selection.getParagraphInfo(selection.start);
            var lineInfo = selection.getLineInfoBasedOnParagraph(paragraph, paragraphInfo.offset);
            var removeOffset = offset - 1;
            if (removeOffset < 0) {
                lineInfo.line = lineInfo.line.previousLine;
        ...
        for (var i = rowStartIndex; i <= count; i++) {
            ...
            for (var z = cellSelectionStartIndex; z <= cellSelectionEndIndex; z++) {
                ...
                if (z === cellSelectionStartIndex) {
                    ...
                    startParagraph = this.documentHelper.getFirstParagraphInCell(startCell_1).childWidgets[0];
                }
                var editStart = this.addEditElement(user);
                if (z === cellSelectionStartIndex) {
                    if (this.editorHistory && this.editorHistory.currentHistoryInfo) {
