...
import { Rect, ImageElementBox, LineWidget, ParagraphWidget, BodyWidget, TextElementBox, ListTextElementBox, TableRowWidget, TableWidget, TableCellWidget, FieldElementBox, TabElementBox, BlockWidget, CommentCharacterElementBox, ShapeElementBox, EditRangeStartElementBox, ShapeBase, TextFrame } from './page';
    ...
    Renderer.prototype.render = function (page, bodyWidget) {
        ...
        if (!(this.viewer instanceof WebLayoutViewer) && bodyWidget.sectionFormat.columns.length > 1) {
            ...
            if (colIndex === 0) {
                ...
                width = HelperMethods.convertPointToPixel(page.bodyWidgets[colIndex].sectionFormat.leftMargin) + bodyWidget.sectionFormat.columns[colIndex].width + (bodyWidget.sectionFormat.columns[colIndex].space / 2);
            }
            else if (colIndex === bodyWidget.sectionFormat.columns.length - 1) {
                xPos = page.bodyWidgets[colIndex].x - (bodyWidget.sectionFormat.columns[colIndex - 1].space / 2);
                width = HelperMethods.convertPointToPixel(page.bodyWidgets[colIndex].sectionFormat.rightMargin) + bodyWidget.sectionFormat.columns[colIndex].width + (bodyWidget.sectionFormat.columns[colIndex - 1].space / 2);
            }
            else {
                xPos = page.bodyWidgets[colIndex].x - (bodyWidget.sectionFormat.columns[colIndex].space / 2);
                width = bodyWidget.sectionFormat.columns[colIndex].width + bodyWidget.sectionFormat.columns[colIndex].space;
            }
        ...
        if (paraWidget.isInsideTable) {
            ...
            if (this.viewer instanceof WebLayoutViewer) {
                ...
                if (section instanceof BodyWidget && section.sectionFormat.columns.length > 1) {
                    var colIndex = section.page.bodyWidgets.indexOf(section);
