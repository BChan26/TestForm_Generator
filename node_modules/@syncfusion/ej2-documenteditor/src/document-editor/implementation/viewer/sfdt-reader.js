...
var SfdtReader = /** @class */ (function () {
    ...
    SfdtReader.prototype.parseParagraph = function (data, paragraph, writeInlineFormat, lineWidget) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (inline.hasOwnProperty('text')) {
                ...
                this.parseBody(inline.blocks, footnoteElement.bodyWidget.childWidgets, footnoteElement.bodyWidget, false);
                    ...
                    if (this.documentHelper.fieldStacks.length > 0) {
                        ...
                        if (lineWidgetCount >= 2) {
                            ...
                            if (!isNullOrUndefined(fieldTextElement) && fieldTextElement instanceof TextElementBox && (fieldTextElement.text.match('PAGE') || fieldTextElement.text.match('page'))) {
                                var textField = fieldTextElement.text;
    ...
    SfdtReader.prototype.parseMajorMinorFontScheme = function (sourceFormat, majorMinor) {
        if (!isNullOrUndefined(sourceFormat.fontTypeface) && Object.keys(sourceFormat.fontTypeface).length > 0) {
            var keys = Object.keys(sourceFormat.fontTypeface);
        ...
        if (!isNullOrUndefined(sourceFormat.fontSchemeList) && sourceFormat.fontSchemeList.length > 0) {
            for (var j = 0; j < sourceFormat.fontSchemeList.length; j++) {
                var data = sourceFormat.fontSchemeList[j];
        ...
        if (!isNullOrUndefined(data.columns) && !isNullOrUndefined(data.numberOfColumns) && data.numberOfColumns > 1) {
            ...
            if (data.columns) {
                for (var i = 0; i < data.columns.length; i++) {
                    ...
                    newCol.width = HelperMethods.convertPointToPixel(data.columns[i].width);
                    newCol.space = HelperMethods.convertPointToPixel(data.columns[i].space);
                    newCol.index = i;
    ...
    SfdtReader.prototype.parseColumns = function (wCols, columns) {
        ...
        if (wCols) {
            for (var i = 0; i < wCols.length; i++) {
                var newCol = new WColumnFormat();
                newCol.width = HelperMethods.convertPointToPixel(wCols[i].width);
                newCol.space = HelperMethods.convertPointToPixel(wCols[i].space);
                newCol.index = i;
                columns.push(newCol);
            }
        }
